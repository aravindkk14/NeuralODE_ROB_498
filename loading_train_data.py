...
def collect_data_random(env, num_trajectories=1000, trajectory_length=10):
    ...
    for _ in tqdm(range(num_trajectories)):
      data_traj = {}
      states = np.empty((trajectory_length+1,state_size), dtype = np.float32)
      actions = np.empty((trajectory_length,action_size), dtype = np.float32)
      states[0] = env.reset()
      for i in range(trajectory_length):
        actions[i] = env.action_space.sample()
        states[i+1], _, done, _ = env.step(actions[i])
        if done:
          break
      data_traj['states'] = states
      data_traj['actions'] = actions
      collected_data.append(data_traj)
...
def process_data_single_step(collected_data, batch_size=500):
    ...
    val_loader = None
    # --- Your code here
    dataset_d = SingleStepDynamicsDataset(collected_data)
...
class SingleStepDynamicsDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # --- Your code here
        traj_num = int(item/self.trajectory_length)
        index = item % self.trajectory_length

        sample['state'] = self.data[traj_num]['states'][index]
        sample['action'] = self.data[traj_num]['actions'][index]
        sample['next_state'] = self.data[traj_num]['states'][index+1]
        # ---
        return sample
...
class MultiStepDynamicsDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # --- Your code here
        traj_num = item//self.trajectory_length
        index = item % self.trajectory_length
        ...
        for i in range(self.num_steps):
          multi_action.append(self.data[traj_num]['actions'][index+i])
          multi_state.append(self.data[traj_num]['states'][index+i+1])

        sample['action'] = torch.tensor(np.array(multi_action))
        sample['next_state'] = torch.tensor(np.array(multi_state))
